...
for i in range(row):
    for j in range(col):
        ...
        tmp = tmp.strip()
        if len(tmp) == 3:
            image[i][j][0] = (int(tmp[2], 16)) << 4
            image[i][j][1] = (int(tmp[1], 16)) << 4
            image[i][j][2] = (int(tmp[0], 16)) << 4
        if len(tmp) == 2:
            image[i][j][0] = (int(tmp[1], 16)) << 4
            image[i][j][1] = (int(tmp[0], 16)) << 4
            image[i][j][2] = 0
        if len(tmp) == 1:
            image[i][j][0] = (int(tmp[0], 16)) << 4
            image[i][j][1] = 0
            image[i][j][2] = 0
...
for i in range(row):
    for j in range(col):
        image_one_channel[i][j] = int(
            img_after[i][j][0]) + int(img_after[i][j][1]) + int(img_after[i][j][2])  # 0b 1g 2r
    ...
    for j in range(col):
        if ((i % 2 == 0 and j % 2 == 0) or (i % 2 == 1 and j % 2 == 1)) and i >= 2 and j >= 2 and i <= 7 and j <= 7:
            Uu = (abs(int(image_one_channel[i][j - 1]) - int(image_one_channel[i - 2][j - 1]))
                  + abs(int(image_one_channel[i][j]) -
                        int(image_one_channel[i - 2][j]))
                  + abs(int(image_one_channel[i][j + 1]) -
                        int(image_one_channel[i - 2][j + 1]))
                  + abs(int(image_one_channel[i - 1][j]) -
                        int(image_one_channel[i + 1][j]))
                  )/4
            ...
            Ul = (abs(int(image_one_channel[i - 1][j]) - int(image_one_channel[i - 1][j - 2]))
                  + abs(int(image_one_channel[i][j]) -
                        int(image_one_channel[i][j - 2]))
                  + abs(int(image_one_channel[i + 1][j]) -
                        int(image_one_channel[i + 1][j - 2]))
                  + abs(int(image_one_channel[i][j + 1]) -
                        int(image_one_channel[i][j - 1]))
                  )/4
            ...
            Gu = int(image_one_channel[i - 1][j]) + abs(
                int(image_one_channel[i][j]) - int(image_one_channel[i - 2][j]))/2
            Gd = int(image_one_channel[i + 1][j]) + abs(
                int(image_one_channel[i][j]) - int(image_one_channel[i + 2][j]))/2
            Gl = int(image_one_channel[i][j - 1]) + abs(
                int(image_one_channel[i][j]) - int(image_one_channel[i][j - 2]))/2
            Gr = int(image_one_channel[i][j + 1]) + abs(
                int(image_one_channel[i][j]) - int(image_one_channel[i][j + 2]))/2
            if Uu == 0 or Ud == 0 or Ul == 0 or Ur == 0:
                Uu = Uu + 1
                Ud = Ud + 1
                Ul = Ul + 1
                Ur = Ur + 1
    ...
    for j in range(col):
        if (i % 2 == 1 and j % 2 == 1) and i >= 2 and j >= 2 and i <= 7 and j <= 7:
            dRG = ((int(img_after[i - 1][j - 1][2]) - int(img_after[i - 1][j - 1][1]))
                   + (int(img_after[i - 1][j + 1][2]) -
                      int(img_after[i - 1][j + 1][1]))
                   + (int(img_after[i + 1][j - 1][2]) -
                      int(img_after[i + 1][j - 1][1]))
                   + (int(img_after[i + 1][j + 1][2]) -
                      int(img_after[i + 1][j + 1][1]))
...
for i in range(row):
    for j in range(col):
        if (i % 2 == 0 and j % 2 == 0) and i >= 2 and j >= 2 and i <= 7 and j <= 7:
            dBG = ((int(img_after[i - 1][j - 1][0]) - int(img_after[i - 1][j - 1][1]))
                   + (int(img_after[i - 1][j + 1][0]) -
                      int(img_after[i - 1][j + 1][1]))
                   + (int(img_after[i + 1][j - 1][0]) -
                      int(img_after[i + 1][j - 1][1]))
                   + (int(img_after[i + 1][j + 1][0]) -
...
for i in range(row):
    for j in range(col):
        if ((i % 2 == 1 and j % 2 == 0) or (i % 2 == 0 and j % 2 == 1)) and i >= 2 and j >= 2 and i <= 7 and j <= 7:
            dRG = ((int(img_after[i - 1][j][2]) - int(img_after[i - 1][j][1]))
                   + (int(img_after[i + 1][j][2]) -
                      int(img_after[i + 1][j][1]))
                   + (int(img_after[i][j - 1][2]) -
                      int(img_after[i][j - 1][1]))
                   + (int(img_after[i][j + 1][2]) -
                      int(img_after[i][j + 1][1]))
                   )/4
            if img_after[i][j][2] != 0:
...
for i in range(row):
    for j in range(col):
        if ((i % 2 == 1 and j % 2 == 0) or (i % 2 == 0 and j % 2 == 1)) and i >= 2 and j >= 2 and i <= 7 and j <= 7:
            dBG = ((int(img_after[i - 1][j][0]) - int(img_after[i - 1][j][1]))
                   + (int(img_after[i + 1][j][0]) -
                      int(img_after[i + 1][j][1]))
                   + (int(img_after[i][j - 1][0]) -
                      int(img_after[i][j - 1][1]))
                   + (int(img_after[i][j + 1][0]) -
                      int(img_after[i][j + 1][1]))
                   )/4
...
for i in range(row):
    for j in range(col):
        img[i][j][0] = (img_after[i][j][0] / 16)
        img[i][j][1] = (img_after[i][j][1] / 16)
        img[i][j][2] = (img_after[i][j][2] / 16)
        img_after[i][j][0] = img[i][j][0]*16
        img_after[i][j][1] = img[i][j][1]*16
        img_after[i][j][2] = img[i][j][2]*16
