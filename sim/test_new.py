...
image = cv2.resize(image, (400,300)) # initail img
    ...
    for j in range(col) :
        if ((i % 2 == 0 and j % 2 == 0) or (i % 2 == 1 and j % 2 == 1)) and i >= 2 and j >= 2 and i <= 297 and j <= 397:
            ...
            if(img_after[i][j][1] != 0) :
                print("!!!")
            if int(Gu*Wu + Gd * Wd + Gl * Wl + Gr * Wr) > 15:
                img_after[i][j][1] = 15
...
for i in range(row) :
    for j in range(col) :
        if (i % 2 == 1 and j % 2 == 1) and i >= 2 and j >= 2 and i <= 297 and j <= 397:
            ...
            if int(img_after[i][j][1] + int(dRG)) > 15:
                img_after[i][j][2] = 15
...
for i in range(row) :
    for j in range(col) :
        if (i % 2 == 0 and j % 2 == 0) and i >= 2 and j >= 2 and i <= 297 and j <= 397:
            dBG = ((float(img_after[i - 1][j - 1][0]) - float(img_after[i - 1][j - 1][1]))
                 + (float(img_after[i - 1][j + 1][0]) - float(img_after[i - 1][j + 1][1]))
                 + (float(img_after[i + 1][j - 1][0]) - float(img_after[i + 1][j - 1][1]))
                 + (float(img_after[i + 1][j + 1][0]) - float(img_after[i + 1][j + 1][1]))
            )/4.0
            ...
            if int(img_after[i][j][1] + int(dBG)) > 15:
                img_after[i][j][0] = 15
